...
class MergeJoin : public Operator
{
   ...
   /// The non-join attributes     Ahmed : this is the rest of the triple, check its length
   std::vector<Register*> leftTail,rightTail;

   /// Possible states while scanning the input
   //Ahmed added the first three entries
   enum ScanState {
       dead,
       leftDead,
       rightDead,
   ...
   /// Shadow tuples
   std::vector<unsigned> leftShadow,rightShadow , leftShadowTemp,rightShadowTemp;
   /// The buffer
   ...
   void getAsyncInputCandidates(Scheduler& scheduler);
    ...
    void workFinal(vector<vector<unsigned>* >* v);
   
   
   //Ahmed functions:
    void   clearLeftTail();
    void   clearRightTail();
    unsigned rightNext(bool&);
    void copyTempLeft();
    void copyTempRight();
    void swapTempLeft();
    void swapTempRight();
    bool swapTempRightFlag ;
    bool swapTempLeftFlag ;
    
    int totalLeftBoarder = 0;
    int totalRightBoarder = 0;
};
//---------------------------------------------------------------------------
#endif
