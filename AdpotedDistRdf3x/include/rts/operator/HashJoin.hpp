...
class HashJoin : public Operator
{
   ...
   struct Entry {
      ...
      //Ahmed: pointer to left buffer entery
      unsigned * leftBordervalue = 0;
   ...
   //Ahmed function
   bool getBorderLeftBufferTail();
   bool getBorderRightBufferTail();
   unsigned resetLeftTail();
   unsigned resetRightTail();
   std::vector<std::vector<Register*>* > rightBorderBuffer;
   unsigned lastBorderRightIndex;
   bool rightHandDone;
 
 unsigned *  addToLeftBorderBuffer(std::vector<Register*> &leftTail, Register* );
 void addToRightBorderBuffer();
 bool getBorderBufferTail();
 bool rightContainBoarder();
 unsigned lastBorderLeftIndex ;
   
   std::vector<std::vector<Register*>* > leftBorderBuffer;
   
   //Ahmed code:
   void sendIntermediateResults(Runtime&);
   bool isFinished();
   void workFinal(vector<vector<unsigned>* >* );
   //Ahmed temp var
      int leftTempCount , rightTempCount;
