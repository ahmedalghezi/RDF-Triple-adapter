   ...
   // Transform a subquery
{
   ...
   for (std::vector<SPARQLParser::Pattern>::const_iterator iter=group.patterns.begin(),limit=group.patterns.end();iter!=limit;++iter) {
      ...
      //Ahmed code intialization of constansts
      node.subject = SemanticAnalysis::specialID ; node.predicate = SemanticAnalysis::specialID; node.object = SemanticAnalysis::specialID;
      bool flagSubj =  encode(dict,diffIndex,(*iter).subject,node.subject,node.constSubject);
      bool flagPred = encode(dict,diffIndex,(*iter).predicate,node.predicate,node.constPredicate);
      bool flagObj = encode(dict,diffIndex,(*iter).object,node.object,node.constObject);
      if (!flagSubj||
          (!flagPred)||
          (!flagObj)) {
         // A constant could not be resolved. This will produce an empty result
          std::cout<<"Ahmed : a constant could not be found, the return false is disabled semanticAnalysis.php 326"<<std::endl;
         //return false;
      }
...
#ifndef SPECILAID_
#define SPECILAID_
//   unsigned SemanticAnalysis::specialID;
#endif
   //Ahmed code two lines: 
   lookup(dict,diffIndex,"ahmed_alghezi",Type::Literal,0,specialID);
   if(specialID == 0)
       specialID -= 1;
   specialID2 = 0;
   lookup(dict,diffIndex,"ahmed_alghezi2",Type::Literal,0,specialID2);
   if(specialID2 == 0)
       specialID2 -= 1;
   
   specialCompressID = 0;
   lookup(dict,diffIndex,"compressed_tripless",Type::Literal,0,specialCompressID);
   if(specialCompressID == 0)
       specialCompressID -= 1;
   
   
   
   
   std::string str1 = "http://mpii.de/yago/resource/Merc%C3%A8_Pons"; 
   std::string str2 = "http://mpii.de/yago/resource/Actrius";
   std::cout<<"the special ID is"<<specialID<<" -- "<<specialID2<<std::endl;
  
   unsigned lookingID = 0;std::string reg = "http://mpii.de/yago/resource/wordnet_instrumentality_103575240";	
          lookup(dict,diffIndex,str1,Type::URI,0,lookingID);
          std::cout<<" the id of str2 is :"<<lookingID<<std::endl<<std::endl;
          lookup(dict,diffIndex,str2,Type::URI,0,lookingID);
          std::cout<<" the id of str1 is :"<<lookingID<<std::endl<<std::endl;
